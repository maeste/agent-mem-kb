...
## [W31/2026] 2026-08-04 | ingest | Cron daily fetch + ingest. Inbox-fetcher failed on qwen.ai (CSR-only site, trafilatura extraction empty). Reverse-engineered the site's API (/api/v2/article/?path=qwen3.8&language=en-US&type=qwen_ai) from JS bundles and pulled full article content manually. Created raw/web/qwen3-8-max/ + wiki/sources/qwen3-8-max.md. Source: Qwen3.8-Max (2.4T/95B MoE, first Max-class with announced open weights; self-evolving harness via oh-my-cli, 16-day autonomous coding; paper reproduction + improvement +2.71pt AIME24; beat 87% of human teams in 24h competition; autonomous RTL/hardware design 8298→678 gates; 365-day e-commerce sim ¥416k; multimodal visual feedback loop). Updated harness-design page (+6th vision: self-producing harness — model generates its own issue state machine/dispatcher/CI loop; goes beyond Miessler's WHAT/HOW). Updated moe-sparsity page (+2.4T/95B = ~4% active, highest active param count in table). Index timeline W31 updated. Inbox marked done. 6 files touched.

## [W31/2026] 2026-08-03 | cron | Daily fetch re-ran against 9 URLs already ingested Aug 1 (inbox checkbox state was lost — URLs re-added as unchecked after manual ingest). All 9 fetched cleanly this run (no Playwright failures). Diff vs existing raw/ is cosmetic only (list formatting, fetched date). No new sources to ingest: all 9 wiki/sources/ pages + wiki/pages/ updates already exist from Aug 1. Deduped inbox.md "Done" entries. No vault content changes.

## [W31/2026] 2026-08-08 | weekly | Weekly review W31 riscritta completamente. La versione precedente (1 ago) elencava fonti W30 con date e periodo errati (26 lug-1 ago). Nuova versione: periodo corretto 1-8 ago, 16 fonti reali W31 (ingested: 2026-W31), 2 pagine modificate (harness-design +7 visioni da 5, moe-sparsity +Qwen/ByteDance). 6 connessioni documentate: spettro harness (auto-prodotto Qwen ↔ externalizzato LoopX + continual Prime/Muse + standalone Kiro), self-improvement (3 path indipendenti), self-reflection debunked, agent security emergente, 4 paradigmi memoria, scale race. Aggiornata index timeline W31 (16 sources, 2 pages). hot.md riscritto. 42 sources totali.
